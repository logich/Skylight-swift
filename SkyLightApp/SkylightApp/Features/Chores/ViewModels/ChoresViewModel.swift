...
final class ChoresViewModel: ObservableObject {
    ...
    func loadChores() async {
        ...
        // Get chores for the next 30 days plus any late chores
        let after = Date()
        let before = Calendar.current.date(byAdding: .day, value: 30, to: after) ?? after

        do {
            chores = try await choresService.getChores(
                frameId: frameId,
                after: after,
                before: before,
                includeLate: true
